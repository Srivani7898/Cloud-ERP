...
export default function EmployeeLeavePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <LeaveForm />
      <HrTable title="My leave requests" description="Leave status and approval history." headers={["Request", "Type", "Dates", "Days", "Status"]} rows={leaveRequests.map((row) => [row.id, row.type, `${row.from} - ${row.to}`, row.days, <HrStatusBadge key="status" status={row.status} />])} />
    </div>
  );
}
