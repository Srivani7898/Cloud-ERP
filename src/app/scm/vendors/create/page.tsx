...
export default function CreateVendorPage() {
  ...
  const [message, setMessage] =
    useState(
      "Enter vendor master details and create a supplier record."
    );
  ...
  async function createVendor() {
    if (
      ...
      setMessage(
        "Please fill Vendor, Contact, Email, and Region before creating the vendor."
      );
      return;
    }
    ...
    try {
      ...
      if (json?.success) {
        ...
        setMessage(
          `${vendorName} was created and synced to the SCM vendor API.`
        );
      } else {
        setMessage(
          "Vendor was not created. Check the API response."
        );
      }
    } catch {
      setMessage(
        "Unable to create vendor. Check the dev server and try again."
      );
    } finally {
      setLoading(false);
    }
