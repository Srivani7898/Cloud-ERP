...
import { Bell, Lock, Mail, Network, ShieldCheck } from "lucide-react";

export default function NotificationsLoginPage() {
  const router = useRouter();
  const [tenant, setTenant] = useState("northstar");
  const [email, setEmail] = useState("notifications.manager@northstar.example");
  ...
  function submitMfa(event: React.FormEvent<HTMLFormElement>) {
    ...
    localStorage.setItem(
      ...
      JSON.stringify({
        state: {
          user: {
            id: "user-notification-manager",
            name: "Notification Manager",
            email,
            tenantId: tenant,
            roles: ["notification_manager"],
          },
          accessToken: "demo-notification-access-token",
          refreshToken: "demo-notification-refresh-token",
          isAuthenticated: true,
          postLoginRedirect: "/notifications/dashboard",
        },
        version: 0,
      })
    );
  ...
  return (
    <main className="min-h-screen bg-[#020617] px-6 py-10 text-white">
      <div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_18%_20%,rgba(59,130,246,0.24),transparent_32%),radial-gradient(circle_at_82%_34%,rgba(139,92,246,0.22),transparent_34%),linear-gradient(rgba(148,163,184,0.06)_1px,transparent_1px),linear-gradient(90deg,rgba(148,163,184,0.06)_1px,transparent_1px)] bg-[size:auto,auto,70px_70px,70px_70px]" />
      <section className="relative mx-auto flex min-h-[calc(100vh-5rem)] max-w-xl items-center">
        <div className="w-full rounded-2xl border border-slate-700 bg-slate-900/75 p-8 shadow-2xl shadow-blue-950/40 backdrop-blur">
          ...
          <div className="mt-6 text-center">
            <p className="text-sm font-semibold uppercase tracking-[0.28em] text-cyan-300">Notification Manager</p>
            <h1 className="mt-3 text-3xl font-semibold">Sign in to Notification Engine</h1>
            <p className="mt-2 text-slate-300">Manage ERP alerts, delivery tracking, templates, rules, channels, and retries.</p>
          </div>
          ...
          {step === "credentials" ? (
            ...
            <form onSubmit={submitMfa} className="mt-8 space-y-5">
              ...
              {error ? <p className="text-sm text-red-300">{error}</p> : null}
              <button className="w-full rounded-lg bg-blue-600 px-4 py-3 font-semibold transition hover:bg-blue-500">Open notification dashboard</button>
