...
const nav = [
  ...
  { href: "/hr/leave", label: "Leave", icon: CalendarCheck },
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-950 dark:bg-slate-950 dark:text-slate-50">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-72 border-r border-slate-200 bg-white/85 p-4 backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/85 xl:block">
        <Link href="/hr/dashboard" className="flex items-center gap-3 px-2 py-3 font-semibold">
        ...
        <div className="mt-5 rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-white/10 dark:bg-white/[0.06]">
          <p className="text-xs uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">People tenant</p>
          <p className="mt-2 font-semibold">Northstar Manufacturing</p>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Global workforce · FY2026</p>
        </div>
      ...
      <div className="xl:pl-72">
        <header className="sticky top-0 z-20 flex min-h-16 flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-white/85 px-4 py-3 backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/85 sm:px-6">
          <div>
            <p className="text-xs uppercase tracking-[0.18em] text-blue-600 dark:text-cyan-300">Human Resources</p>
            <h1 className="text-lg font-semibold tracking-normal">Enterprise people operations center</h1>
          </div>
          <div className="flex items-center gap-2">
            <ThemeToggle />
            <Button asChild variant="outline" size="sm"><Link href="/auth/logout"><LogOut className="h-4 w-4" />Sign out</Link></Button>
          </div>
